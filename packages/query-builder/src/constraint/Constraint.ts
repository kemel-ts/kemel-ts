...
import { Punctuation, Query, Raw } from "..";
...
export class Constraint {
  ...
  write(writer: Writer, addType: boolean = true) {
    addType && writer.writeWithBeginSpace(this.type);
    this.leftValue.write(writer);
    if (this.leftValue instanceof SubConstraint) return;

    writer.writeWithBeginSpace(this.operator);
  ...
  private writeBetween(writer: Writer) {
    ...
    writer.writeWithBeginSpace(Type.And);
